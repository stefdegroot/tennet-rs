...
pub fn sync_service (app_state: AppState) {
    ...
    let _ = schedule_tasks(ScheduleGranularity::SECONDS, &[10],app_state.clone(), balance_delta_service, "balance_delta");
    let _ = schedule_tasks(ScheduleGranularity::MINUTES, &[1, 16, 31, 46],app_state.clone(), merit_order_service, "merit_order");
    let _ = schedule_tasks(ScheduleGranularity::MINUTES, &[6],app_state.clone(), settlement_prices_service, "settlement_prices");
